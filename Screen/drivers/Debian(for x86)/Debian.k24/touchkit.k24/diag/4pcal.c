...
{	
	...
	if (_25Mode)
	{	CorrectPoint(&coef.mapping, &rx, &ry);
		Calibration25Pt(&coef, &rx, &ry);  
	}
...
{	int i, j;
	...
	{	int ij2addr[4][2] = 
		...
		for (i=0; i<_N*_N; i++)
		{	for (j=0; j<2; j++)
			{	eeprom[ij2addr[i][j]] = (short)PtsValue[i][j];
			}
		}
		#define RETRY	5
		...
		FixEEPROM(eeprom);
		for (i=0; i<RETRY; i++)
		{	if (!_WriteEEPROM(deviceFD, eeprom, EPR_LLX, EPR_URY))
			{	break;
			}
		}
		if (i<RETRY)
		{	for (i=0; i<RETRY; i++)
			{	if (!_WriteEEPROM(deviceFD, eeprom, EPR_CheckSum, EPR_CheckSum))
				{	break;
				}
			}
			if (i<RETRY)
