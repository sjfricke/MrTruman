...
void loopOutputSetup() {
  ...
  pcm_buff_size = framesout * channels * 2;   /* 2 bytes/sample, 2 channels */
  printf("size is %d\n", pcm_buff_size);
...
void loopbackSetup() {
  loopOutputSetup(); // need to set pcm_buff_size
  
  pcm_buffer = (void *) malloc(pcm_buff_size);
  if (pcm_buffer == NULL) {
    printf("ERROR: Malloc pcm_buffer");
  }
...
int loopback() {
  ...
  pthread_create(&tid, NULL, analyze_buffer, (void *)pcm_buffer);
  ...
  while ((aux_in = GpioGetValue(aux_pin)) == 1) {
    ...
    snd_pcm_readi(inhandle, pcm_buffer, frames);
